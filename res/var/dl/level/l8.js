...
<ul>
<li><span>Convolution layers consist of a set of learnable filters (or kernels) having small widths and heights and the same depth as that of input volume (3 if the input layer is image input).</span></li>
<li><span>For example, if we have to run convolution on an image with dimensions 34x34x3. The possible size of filters can be axax3, where ‘a’ can be anything like 3, 5, or 7 but smaller as compared to the image dimension.</span></li>
<li>During the forward pass, we slide each filter across the whole input volume step by step where each step is called <strong>stride</strong><span> (which can have a value of 2, 3, or even 4 for high-dimensional images) and compute the dot product between the kernel weights and patch from input volume.</span></li>
<li><span>As we slide our filters we’ll get a 2-D output for each filter and we’ll stack them together as a result, we’ll get output volume having a depth equal to the number of filters. The network will learn all the filters.</span></li>
</ul>
`; //message
...
<ul>
<li><b><strong>Input Layers:</strong></b><span> It’s the layer in which we give input to our model. In CNN, Generally, the input will be an image or a sequence of images. This layer holds the raw input of the image with width 32, height 32, and depth 3.</span></li>
<li><b><strong>Convolutional Layers: </strong></b><span>This is the layer, which is used to extract the feature from the input dataset. It applies a set of learnable filters known as the kernels to the input images. The filters/kernels are smaller matrices usually 2×2, 3×3, or 5×5 shape. it slides over the input image data and computes the dot product between kernel weight and the corresponding input image patch. The output of this layer is referred ad feature maps. Suppose we use a total of 12 filters for this layer we’ll get an output volume of dimension 32 x 32 x 12.</span></li>
<li><b><strong>Activation Layer: </strong></b><span>By adding an activation function to the output of the preceding layer, activation layers add nonlinearity to the network. it will apply an element-wise activation function to the output of the convolution layer. Some common activation functions are </span><b><strong>RELU</strong></b><span>: max(0, x), &nbsp;</span><b><strong>Tanh</strong></b><span>, </span><b><strong>Leaky RELU</strong></b><span>, etc. The volume remains unchanged hence output volume will have dimensions 32 x 32 x 12.</span></li>
<li><b><strong>Pooling layer:</strong></b><span> This layer is periodically inserted in the covnets and its main function is to reduce the size of volume which makes the computation fast reduces memory and also prevents overfitting. Two common types of pooling layers are </span><b><strong>max pooling</strong></b><span> and </span><b><strong>average pooling</strong></b><span>. If we use a max pool with 2 x 2 filters and stride 2, the resultant volume will be of dimension 16x16x12.&nbsp;</span></li>
</ul>
${ShowImage('32.png')}
<ul>
<li><b><strong>Flattening: </strong></b><span>The resulting feature maps are flattened into a one-dimensional vector after the convolution and pooling layers so they can be passed into a completely linked layer for categorization or regression.</span></li>
<li><b><strong>Fully Connected Layers: </strong></b><span>It takes the input from the previous layer and computes the final classification or regression task.</span></li>
</ul>
${ShowImage('33.jpg')}
<ul>
